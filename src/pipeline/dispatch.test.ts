...
describe("dispatchInboundMessage – searchManager passthrough", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("passes searchManager to runAgent when provided", async () => {
    ...
    const fakeSearchManager = { search: vi.fn() } as unknown as MemorySearchManager;

    await dispatchInboundMessage(makeMsgContext(), {
      config,
      sessions,
  ...
  it("passes undefined searchManager when not provided", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");
...
describe("dispatchInboundMessage – envelope wrapping", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("wraps the prompt with a message envelope containing channel and sender", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");
    ...
    // The prompt should be envelope-wrapped, not raw text
    // Channel IDs are lowercase (e.g. "telegram"), envelope preserves original casing
    expect(call.prompt).toContain("[telegram Tommy");
    expect(call.prompt).toContain("] what time is it?");
    expect(call.prompt).not.toBe("what time is it?");
  });

  it("omits elapsed time on first message (when lastActiveAt === createdAt)", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");

    const config = createTestConfig({ timezone: "UTC" });
    // New session — sessions.get returns undefined, so dispatch creates fresh one
    const sessions = createMockSessionStore();

    await dispatchInboundMessage(makeMsgContext({ timestamp: TS }), { config, sessions });

    const call = vi.mocked(runAgent).mock.calls[0][0];
    // First message should NOT have +elapsed
    expect(call.prompt).not.toMatch(/\+\d+[smhd]/);
  });

  it("includes elapsed time on subsequent messages", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");

    const config = createTestConfig({ timezone: "UTC" });
    // Pre-existing session with lastActiveAt 5 minutes ago
    const fiveMinAgo = TS - 300_000;
    const existingSession: SessionEntry = {
      sessionId: "sess-1",
      sessionKey: "test:dm:user1",
      agentId: "default",
      channel: "terminal",
      createdAt: fiveMinAgo - 3_600_000, // created 1h+5m ago
    ...
    const sessions = createMockSessionStore({ "test:dm:user1": existingSession });

    await dispatchInboundMessage(makeMsgContext({ timestamp: TS }), { config, sessions });

    const call = vi.mocked(runAgent).mock.calls[0][0];
    // Should have +5m elapsed
    expect(call.prompt).toContain("+5m");
  });

  it("passes sessions store to runAgent", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");
    ...
    expect(runAgent).toHaveBeenCalledWith(expect.objectContaining({ sessions }));
  });
});

// ── cronService passthrough ─────────────────────────────────────────────

describe("dispatchInboundMessage – cronService passthrough", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("passes cronService to runAgent when provided", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");

    const config = createTestConfig();
    const sessions = createMockSessionStore();
    const fakeCron = { start: vi.fn(), stop: vi.fn() } as unknown as CronService;

    await dispatchInboundMessage(makeMsgContext(), {
      config,
      sessions,
      cronService: fakeCron,
    });

    expect(runAgent).toHaveBeenCalledWith(expect.objectContaining({ cronService: fakeCron }));
  });

  it("passes undefined cronService when not provided", async () => {
    const { runAgent } = await import("../agents/runner.js");
    const { dispatchInboundMessage } = await import("./dispatch.js");
    ...
    expect(runAgent).toHaveBeenCalledWith(expect.objectContaining({ cronService: undefined }));
  });
});
